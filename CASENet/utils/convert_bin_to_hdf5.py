...
def convert_num_to_bitfield(label_data, h, w, npz_name, root_folder, h5_file, cls_num=19):
    ...
    for n in label_list: # Iterate in each pixel
        ...
        if actual_len > 1:
            print(actual_len, bit_tensor, torch.zeros(cls_num-actual_len), torch.zeros(cls_num-actual_len).byte())
            exit()
...
if __name__ == "__main__":
    f = open("cityscapes-preprocess/data_proc/val.txt", 'r')
    lines = f.readlines()
    root_folder = "cityscapes-preprocess/data_proc"

    h5_file = h5py.File("utils/ma_label_binary_np.h5", 'w')
    for ori_line in tqdm(lines):
        line = ori_line.split()
        bin_name = line[1]
        img_name = line[0]
        
        label_path = os.path.join(root_folder, bin_name[1:]) 
        img_path = os.path.join(root_folder, img_name[1:])
